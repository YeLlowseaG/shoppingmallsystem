...
VALUES (55, 3, '订单问题', 1, 'message', 'order/OrderMessage', 'ChatLineRound', 'admin:order:message:list', 2, 1, 0, NOW(), NOW())
...
SELECT 1, 55
WHERE NOT EXISTS (SELECT 1 FROM `sys_role_menu` WHERE `role_id` = 1 AND `menu_id` = 55);

-- 3. 为运营人员角色分配订单问题菜单权限（如果存在）
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`)
SELECT 2, 55
WHERE NOT EXISTS (SELECT 1 FROM `sys_role_menu` WHERE `role_id` = 2 AND `menu_id` = 55)
AND EXISTS (SELECT 1 FROM `sys_role` WHERE `id` = 2);

-- 4. 为客服人员角色分配订单问题菜单权限（如果存在，role_id=5）
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`)
SELECT 5, 55
WHERE NOT EXISTS (SELECT 1 FROM `sys_role_menu` WHERE `role_id` = 5 AND `menu_id` = 55)
