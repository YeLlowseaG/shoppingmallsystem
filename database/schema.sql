...
CREATE TABLE IF NOT EXISTS `sys_user` (
  ...
  `user_level` VARCHAR(20) DEFAULT '普通' COMMENT '用户等级（普通/VIP/金牌）',
  `status` VARCHAR(20) DEFAULT '待审核' COMMENT '状态（待审核/已激活/已禁用）',
...
CREATE TABLE IF NOT EXISTS `sys_user_audit` (
  ...
  `audit_status` VARCHAR(20) NOT NULL COMMENT '审核状态（待审核/已通过/已拒绝）',
...
CREATE TABLE IF NOT EXISTS `product_price` (
  ...
  `user_level` VARCHAR(20) NOT NULL COMMENT '用户等级（普通/VIP/金牌）',
...
CREATE TABLE IF NOT EXISTS `order` (
  ...
  `order_status` VARCHAR(20) DEFAULT '待付款' COMMENT '订单状态（待付款/已付款未发货/已发货/已完成/已取消/已退款/已退货）',
  `payment_method` VARCHAR(50) DEFAULT NULL COMMENT '支付方式（支付宝/微信/预存款/线下支付）',
...
CREATE TABLE IF NOT EXISTS `payment_record` (
  ...
  `payment_method` VARCHAR(50) NOT NULL COMMENT '支付方式（支付宝/微信/预存款/线下支付）',
  `amount` DECIMAL(10,2) NOT NULL COMMENT '支付金额',
  `payment_status` VARCHAR(20) DEFAULT '待支付' COMMENT '支付状态（待支付/已支付/已退款/已失败）',
...
CREATE TABLE IF NOT EXISTS `message` (
  ...
  `message_type` VARCHAR(20) DEFAULT '普通' COMMENT '消息类型（普通/系统/订单/其他）',
