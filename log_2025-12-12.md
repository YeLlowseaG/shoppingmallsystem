...
### 完善购物车结算页面与商品模块对接并屏蔽不需要的字段
- **需求描述**：
  1. 结算页面需要根据目前购物车、商品模块最新的代码来对接字段，需要对接后端联调
  2. 结算页面，屏蔽送货日期、时间字段
  3. 税金(0%)、发票抬头字段也不需要
  4. 结算页面，选中配送方式模块，也先屏蔽掉
- **修改内容**：
  1. **后端修改（OrderServiceImpl）**：
     - 注入 `ProductRepository`、`ProductPriceRepository` 和 `UserRepository` 依赖
     - 完善 `createOrder()` 方法：
       - 查询用户等级
       - 查询商品信息（名称、编码、图片、重量、库存等）
       - 验证商品是否存在且已上架
       - 验证库存是否充足
       - 根据用户等级查询对应的价格（支持阶梯价格）
       - 使用会员价计算订单金额
       - 创建订单商品快照，保存商品信息
     - 添加 `getPriceByUserLevel()` 辅助方法：根据商品ID、用户等级和数量查询价格
     - 添加 `convertUserLevelToString()` 辅助方法：将用户等级数字转换为字符串
  2. **前端修改（Checkout.vue）**：
     - 屏蔽送货日期、时间字段（注释掉相关代码）
     - 屏蔽税金(0%)、发票抬头字段（注释掉相关代码）
     - 屏蔽配送方式模块（注释掉相关代码和对话框）
     - 修改订单创建请求，不再传递已屏蔽的字段
- **字段对接说明**：
  - 商品信息：从 `Product` 表查询（名称、编码、主图、重量、库存等）
  - 价格计算：根据用户等级从 `ProductPrice` 表查询，支持阶梯价格
  - 订单商品快照：保存商品名称、编码、图片、价格、重量等信息
- **修改文件**：
  - `backend/src/main/java/com/shoppingmall/service/buyer/impl/OrderServiceImpl.java`
  - `frontend/src/views/cart/Checkout.vue`
- **注意事项**：
  - 订单创建时会验证商品是否存在、是否上架、库存是否充足
  - 价格根据用户等级自动计算，支持阶梯价格
  - 已屏蔽的字段代码保留在注释中，便于后续恢复

### 屏蔽购物车页面的快速添加商品模块
- **需求描述**：屏蔽购物车页面中"已放入购物车的商品"上面的快速添加商品模块（货号、数量、添加到购物车这一行）
- **修改内容**：
  1. **前端修改**：
     - 在 `frontend/src/views/cart/Index.vue` 中，将快速添加商品区（`quick-add-section`）注释掉
     - 保留代码以便后续需要时可以恢复
- **修改文件**：
  - `frontend/src/views/cart/Index.vue`
- **说明**：
  - 快速添加商品功能已隐藏，但代码保留在注释中，如需恢复可直接取消注释

### 购物车页面商品名称支持点击跳转到商品详情页
- **需求描述**：购物车页面的商品名称需要支持点击跳转到商品详情页面
- **修改内容**：
  1. **前端修改**：
     - 在 `frontend/src/views/cart/Index.vue` 中，将商品名称从普通文本改为 `router-link` 链接
     - 链接跳转路径：`/products/${item.productId}`
     - 添加商品名称链接的样式：
       - 默认颜色：`#333`
       - hover时颜色：`#409eff`（Element Plus主题色）
       - hover时显示下划线
       - 添加过渡动画效果
- **修改文件**：
  - `frontend/src/views/cart/Index.vue`
- **用户体验**：
  - 商品名称现在可以点击，鼠标悬停时有视觉反馈
  - 点击后跳转到对应的商品详情页面

### 完善购物车页面与商品模块的字段对接
- **需求描述**：购物车页面需要根据商品模块对接字段，特别是商品名称、图片等字段
- **问题分析**：
  - `CartServiceImpl.convertToVO()` 方法中使用的是临时数据，未从商品表查询真实信息
  - `addToCartByCode()` 方法未实现，无法通过商品编码添加商品到购物车
  - 购物车VO中缺少商品名称、图片、编码、重量等字段的真实数据
- **修改内容**：
  1. **CartServiceImpl 修改**：
     - 注入 `ProductRepository` 和 `ProductPriceRepository` 依赖
     - 完善 `convertToVO()` 方法：
       - 从商品表查询商品信息（名称、编码、主图、重量等）
       - 根据用户等级查询对应的价格（销售价、会员价、批发价）
       - 支持阶梯价格查询（根据数量范围）
       - 处理商品不存在的情况，设置默认值
     - 完善 `addToCartByCode()` 方法：
       - 通过商品编码查询商品
       - 验证商品是否存在且已上架
       - 调用 `addToCart()` 方法添加到购物车
     - 添加 `getPriceByUserLevel()` 辅助方法：
       - 根据商品ID、用户等级和数量查询价格
       - 支持阶梯价格匹配
     - 添加 `convertUserLevelToString()` 辅助方法：
       - 将用户等级数字（0/1/2）转换为字符串（普通/VIP/金牌）
- **字段对接说明**：
  - `productCode`：从 `Product.productCode` 获取
  - `name`：从 `Product.productName` 获取
  - `image`：从 `Product.mainImage` 获取（如果没有主图则设置为空字符串）
  - `weight`：从 `Product.weight` 获取（单位：克）
  - `salesPrice`：从 `Product.salePrice` 或 `Product.basePrice` 获取
  - `memberPrice`：根据用户等级从 `ProductPrice` 表查询，如果没有则使用销售价
  - `wholesalePrice`：查询金牌等级的价格（如果有）
- **修改文件**：
  - `backend/src/main/java/com/shoppingmall/service/buyer/impl/CartServiceImpl.java`
- **注意事项**：
  - 商品不存在时会显示"商品已下架"，但不会抛出异常
  - 价格查询支持阶梯价格，会根据购买数量匹配对应的价格
  - 用户等级默认为"普通"（如果用户不存在或等级为空）
