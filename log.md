...
## 2025-12-25 修复application-test.yml重复配置问题

### 问题描述
后端服务在宝塔服务器上启动失败，报错：`DuplicateKeyException: found duplicate key spring`。错误显示在第2行和第125行都有 `spring:` 键。

### 问题原因
合并ERP代码时，`application-test.yml` 文件中出现了3个重复的 `spring:` 配置块：
- 第2行：第一个配置块（正确）
- 第125行：第二个重复的配置块
- 第249行：第三个重复的配置块

这导致YAML解析器无法正确解析配置文件。

### 修复内容
1. **修改 `backend/src/main/resources/application-test.yml`**：
   - 删除了第123行之后的所有重复配置内容
   - 只保留第一个完整的配置块（第1-122行）
   - 确保文件中只有一个 `spring:` 键
   - 保留了正确的密码配置值（使用环境变量方式）
...
### 技术细节
- **问题类型**：YAML格式错误（重复键）
- **修复方法**：删除重复的配置块，只保留第一个
- **配置方式**：使用环境变量 `${变量名:默认值}` 格式，便于不同环境配置
