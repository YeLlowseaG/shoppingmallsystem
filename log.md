...
## 2026-01-05 - 调整首页商品分类导航二级菜单位置

### 功能说明
调整首页商品分类导航的二级菜单定位方式，使其紧贴一级菜单，方便用户选择二级分类。

### 修改原因
用户反馈二级菜单距离太远，无法选择二级分类。原因是二级菜单使用了固定定位（fixed），距离一级菜单有较大间隙。

### 修改内容

**文件：** `frontend/src/components/home/Navbar.vue`
- 将二级菜单（`.sub-categories`）的定位方式从 `position: fixed` 改为 `position: absolute`
- 将定位坐标从固定的 `left: 580px; top: 130px;` 改为相对于一级菜单项的 `left: 220px; top: 0;`
- 将 `height: 500px` 改为 `min-height: 100%`，使二级菜单高度自适应一级菜单项的高度

### 影响范围
- ✅ 首页商品分类导航的二级菜单现在紧贴一级菜单右侧显示
- ✅ 用户可以轻松从一级菜单移动到二级菜单，无需跨越间隙
