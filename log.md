...
## 2026-01-07 - 合并功能分支到 dev 并推送到远程

### 操作说明
将 `feature/yellow-modules` 功能分支合并到 `dev` 分支，并推送到 gitee 远程仓库。

### 操作内容
- **源分支**：feature/yellow-modules
- **目标分支**：dev
- **合并方式**：手动解决冲突后合并
- **冲突文件**：log.md（已解决，保留了两边的所有记录）
- **推送状态**：已成功推送到 gitee/dev
...
## 2026-01-07 - 执行订单问题菜单 SQL 脚本

### 操作说明
执行了 `update-20251214-add-order-message-menu.sql` 脚本，添加订单问题管理菜单。

### 执行结果
- **脚本文件**：`database/update-20251214-add-order-message-menu.sql`
- **菜单ID**：55（脚本中为54，但实际创建为55，可能ID 54已被占用）
- **菜单名称**：订单问题
...
### 权限分配
- ✅ 超级管理员角色 (role_id=1) 已分配菜单权限
- ✅ 运营人员角色 (role_id=2) 已分配菜单权限

### 影响范围
- ✅ 管理后台订单管理下已存在"订单问题"子菜单
- ✅ 相关角色已获得菜单访问权限
- ✅ 菜单配置正确，功能可用

## 2026-01-07 - 合并 dev 分支更新到 feature/yellow-modules

### 操作说明
将 gitee 上 dev 分支的最新更新合并到功能分支 `feature/yellow-modules`。

### 合并内容
- **合并方式**：Fast-forward（无冲突）
- **更新统计**：240 个文件被修改，890 行新增，330 行删除
- **主要更新**：
  1. 商品详情页面/购物车页面，购买数量限制不能超过库存的数量
  2. 商品详情页面/购物车页面，未登录状态操作优化，直接跳转到登录页
  3. 会员中心，站内消息模块入口先屏蔽
  4. 增加商品不存在或已被删除的提示页面
  5. 文件结构优化（部分文件移动到 docs/apitest/ 目录）
  6. 删除 `jushuitan_itemsku_upload.jmx` 文件

### 影响范围
- ✅ 功能分支已包含 dev 分支的所有最新更新
- ✅ 无冲突，合并成功
- ✅ 当前分支领先远程分支 5 个提交
...
## 2024-XX-XX - 商品详情和购物车页面库存检查优化

### 修改内容

优化商品详情页面和购物车页面，当用户输入超过库存的数量时，前端限制输入并提示"库存不足"，防止用户购买超过库存数量的商品。

### 具体修改

1. **商品详情页面（Detail.vue）**
   - 动态设置数量输入框的最大值为当前可用库存数量
   - 在加入购物车函数中添加库存检查
   - 如果输入数量超过库存，提示"库存不足"并自动调整数量为最大库存
   - 添加 `getAvailableStock()` 函数获取可用库存（支持SKU和基础库存）

2. **购物车页面（Index.vue）**
   - 动态设置数量输入框的最大值为商品库存（如果后端返回库存信息）
   - 在修改数量函数中添加库存检查
   - 如果后端返回了库存信息，在修改数量前检查是否超过库存
   - 如果后端返回库存不足的错误，显示友好的错误提示
   - 添加 `getItemMaxQuantity()` 函数获取商品的最大可购买数量

3. **购物车API类型定义（cart.ts）**
   - 在 `CartVO` 接口中添加 `stock` 和 `skuId` 可选字段，支持后端返回库存信息

### 技术实现

- 商品详情页面：
  - 使用 `:max="getAvailableStock()"` 动态设置数量输入框的最大值
  - 在 `addToCart` 函数中检查 `quantity.value > availableStock`
  - 如果超过库存，提示错误并自动调整数量为最大库存

- 购物车页面：
  - 使用 `:max="getItemMaxQuantity(item)"` 动态设置数量输入框的最大值
  - 在 `handleQuantityChange` 函数中检查库存
  - 如果后端返回库存信息，在修改前检查；否则依赖后端验证
  - 捕获后端返回的库存不足错误，显示友好提示

### 相关文件

- `frontend/src/views/products/Detail.vue`（修改：添加库存检查和数量限制）
- `frontend/src/views/cart/Index.vue`（修改：添加库存检查和数量限制）
- `frontend/src/api/buyer/cart.ts`（修改：添加库存字段到CartVO接口）

---

## 2024-XX-XX - 购物车总数量点击登录检查优化

### 修改内容

优化顶部导航栏的购物车总数量链接，当用户未登录时，点击购物车总数量时，直接跳转到登录页面，引导用户先登录。

### 具体修改

1. **顶部导航栏（TopBar.vue）**
   - 将购物车总数量的 `router-link` 改为 `a` 标签并添加点击事件处理
   - 添加 `handleCartClick` 函数处理点击逻辑
   - 在点击时检查用户登录状态
   - 如果用户未登录，显示提示并跳转到登录页面
   - 如果用户已登录，正常跳转到购物车页面
...
- 参考商品详情页面的登录检查逻辑，保持代码风格一致

### 相关文件

- `frontend/src/components/home/TopBar.vue`（修改：添加购物车点击登录检查逻辑）

---

## 2024-XX-XX - 购物车页面库存限制完善

### 修改内容

完善购物车页面的库存限制功能，在加载购物车列表时，为每个商品获取库存信息，确保数量输入框能够正确限制不能超过库存数量。

### 具体修改

1. **购物车页面（Index.vue）**
   - 在 `loadCartList` 函数中，为每个购物车商品获取商品详情以获取库存信息
   - 如果商品有SKU且购物车项有skuId，获取对应SKU的库存
   - 如果商品没有SKU，使用商品的基础库存
   - 将获取到的库存信息添加到购物车项中，供数量限制使用
   - 如果获取库存失败，不设置库存限制（依赖后端验证）

### 技术实现

- 使用 `Promise.all` 并行获取所有商品的库存信息，提高加载速度
- 调用 `getProductById` API 获取商品详情
- 根据商品是否有SKU，选择使用SKU库存或商品基础库存
- 如果获取库存失败，使用 `undefined` 作为库存值，数量输入框会使用默认的最大值（999），后端会验证库存
