...
### 功能说明
修复商品更新时，SKU库存被商品总库存平均分配覆盖的问题。当用户明确设置了SKU库存后，更新商品时不应该用商品总库存来覆盖SKU库存。

### 修改文件

#### 后端
1. `backend/src/main/java/com/shoppingmall/service/admin/impl/StockServiceImpl.java` - 注释掉同步SKU库存的逻辑

### 具体修改

#### 问题分析
- **问题现象**：用户在前端修改SKU库存（如：153, 145, 152）并保存后，再次打开编辑时，库存被重置为150
- **根本原因**：在 `StockServiceImpl.updateProductTotalStock()` 方法中，调用了 `syncProductSkuStock()` 方法，该方法会将商品总库存平均分配给所有SKU，覆盖了用户设置的SKU库存
- **错误逻辑**：当商品有SKU时，不应该用商品总库存来覆盖SKU库存，因为：
  1. SKU库存应该由用户明确设置
  2. 商品总库存应该由所有SKU库存的总和计算得出（`ProductSkuServiceImpl.updateProductTotalStock()`）
  3. 不应该用商品总库存来覆盖用户设置的SKU库存

#### 修复方案
- **注释掉同步SKU库存的调用**：在 `updateProductTotalStock()` 方法中，注释掉 `syncProductSkuStock()` 的调用
- **添加说明注释**：解释为什么不再同步SKU库存

### 功能特性
- ✅ SKU库存不会被商品更新操作覆盖
- ✅ 用户设置的SKU库存能够正确保存和显示
- ✅ 商品总库存由所有SKU库存的总和计算得出（正确的逻辑）
- ✅ 保持数据一致性，SKU库存和商品总库存同步更新

### 技术细节
- `syncProductSkuStock()` 方法会将商品总库存平均分配给所有SKU
- 这个逻辑在商品没有SKU库存数据时可能有用，但在用户明确设置了SKU库存时是错误的
- 正确的逻辑是：SKU库存由用户设置，商品总库存由所有SKU库存的总和计算
